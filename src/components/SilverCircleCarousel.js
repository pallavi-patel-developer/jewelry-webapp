...
export default function SilverCircleCarousel() {
  ...
  return (
    <div className="w-auto -mx-6 md:-mx-12 pb-16 relative z-20 overflow-visible select-none">
      ...
      >
        {/* ─── SMOKY EDGE GRADIENTS (Pointer-events-none, soft feathered corners/edges) ─── */}
        <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-[#121212] via-[#121212]/90 to-transparent pointer-events-none z-30" />
        <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#121212] via-[#121212]/90 to-transparent pointer-events-none z-30" />
        <div className="absolute inset-y-0 left-0 w-[450px] bg-gradient-to-r from-[#121212] via-[#121212]/95 to-transparent pointer-events-none z-20" />
        ...
        <div className="absolute inset-0 overflow-hidden rounded-none z-10">
          ...
          <div className="absolute top-12 pb-32 md:pb-0 right-6 md:top-24 md:right-12 z-40 flex gap-3">
            <button
              onClick={handlePrev}
              className="w-10 h-10 pb-32 md:pb-0 md:w-14 md:h-14 rounded-full border border-white/10 bg-white/5 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/15 hover:scale-105 transition-all duration-300 shadow-md"
              aria-label="Previous slide"
            >
              <ChevronLeft className="w-5 h-5 pb-32 md:pb-0 md:w-7 md:h-7" strokeWidth={1.5} />
            </button>
            <button
              onClick={handleNext}
                ...
                return (
                  ...
                  >
                    <div className="block group">
                      {/* Curved/Arched Luxury Card (Upscaled size: w-[190px] md:w-[310px] with mirrored borders) */}
                      <div
                        className={`relative w-[190px] md:w-[310px] aspect-[4/5] bg-[#1a1a1a] rounded-r-[50px] rounded-l-[15px] md:rounded-r-[100px] md:rounded-l-[25px] overflow-hidden shadow-2xl border border-white/10 transition-all duration-500 ${diff === 0
                          ? "ring-2 ring-brand-btn ring-offset-4 ring-offset-[#121212] scale-105"
                          : "opacity-40 scale-95 hover:opacity-75"
                          }`}
                      >
                        {/* Silver Jewel image (object-contain with proper padding ensures the full jewelry item is perfectly visible!) */}
                        <Image
                          src={product.image}
                          alt={product.name}
                          fill
                          sizes="(max-width: 768px) 190px, 310px"
                          className="object-contain p-4 md:p-8 transition-transform duration-700 group-hover:scale-[1.07] brightness-[1.05]"
                        />
                      </div>
                    </div>
                  </div>
